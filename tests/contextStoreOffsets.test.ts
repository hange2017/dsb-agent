...
// 以便断言"按 seq 回查不整读 .context.json"(仅随机读小段)。
...
function seedLarge(store: ContextStore, n: number, sessionId = "big"): void {
...
describe("ContextStore 偏移直读(免整文件解析)", () => {
  ...
  it("大文件回查不整读 .context.json(只随机读小段)", () => {
    const store = new ContextStore(dir);
    seedLarge(store, 300); // 300 × 2KB ≈ 600KB 原文
    const ctxFile = path.join(dir, "big.context.json");
    const sizeBefore = fs.statSync(ctxFile).size;
    // 预热索引缓存,让回查只走原文随机读(readIndex 命中 mtime 缓存,不再读 index 文件)
    store.index("big");
    const readFile = vi.mocked(fs.readFileSync);
    const readSync = vi.mocked(fs.readSync);
    readFile.mockClear();
    readSync.mockClear();
    const hits = store.get("big", [42, 150, 299]);
        expect(hits.map((c) => c.seq)).toEqual([42, 150, 299]);
    expect(hits[0].content).toContain("需求 42");
    // 关键:没有整文件 readFileSync(.context.json),只发生了小段 readSync
    const ctxReads = readFile.mock.calls.filter((c) => String(c[0]).includes("big.context.json"));
  ...
  it("旧格式(无偏移索引)→ get 回退全量并重建带偏移索引,下次回查走直读", () => {
    const store = new ContextStore(dir);
    seedLarge(store, 60, "legacy");
    const ctxFile = path.join(dir, "legacy.context.json");
    // 模拟旧格式:删掉索引文件(相当于旧版只有 .context.json,或手改索引丢 offset)
    ...
    const store2 = new ContextStore(dir); // 新实例,索引缓存空
    ...
    const ctxReads = readFile.mock.calls.filter((c) => String(c[0]).includes("legacy.context.json"));
    expect(ctxReads).toHaveLength(0);
  });

  it("原文被外部改坏导致偏移错位 → 回退全量解析仍返回正确结果", () => {
    const store = new ContextStore(dir);
    store.append("s1", [chunk({ seq: 1, content: "A" }), chunk({ seq: 2, content: "B" })]);
    store.index("s1"); // 预热缓存
    // 外部手改原文(在中间插入字节),索引偏移随之错位
    const ctxFile = path.join(dir, "s1.context.json");
