...
- **降级为单行目标锚**:`ContextManager` 不再构建地图,只维护 `residentGoal`(= `demands[0]`,压缩块需求轨首条,首条免裁故始终可得);任务锚新增 `**会话目标:**` 单行(1 行即可把目标搬到消息尾部,拿到「注意力更强」这一唯一真价值)。锚的其余部分不变:`### 下一步`(来自 `TodoManager` 真实未完成待办)+ 当前清单 + `ContextRecall` 回查入口。
- **设置项 `dsbAgent.compaction.taskMapEnabled` 保留键名**(避免老用户配置漂移),语义改为「单行会话目标锚」;关闭时块字节与旧版一致。
- **向后兼容**:`CompactBlockParts.map` 字段保留 —— 仅用于解析**旧会话**遗留的压缩块,不再写入。
