...
| 缓存前缀稳定性 | ★★★★☆ | P0-P3 规则已固化且代码/测试落地;稳定期命中 68~75%,压缩后首轮 ~10% 仍是旧基线,待 A 方向量化验证 |
...
### 方向 A:缓存前缀稳定性 P2 基线量化(验证雪崩根治收益)🔴 最高优先

- **现状**:P0-P3 代码与测试**已全部落地**(2026-08-16 核验:82 个 context 测试全绿,规则表见 01-architecture §5.1)。**唯一缺口是验证环节**:`scripts/analyze-cache-prefix.py` 的稳定期 68~75% / 压缩后首轮 ~10% 均为 2026-08-10/11 旧基线,P0-P3 全量落地后从未跑过改前改后对比;小时级官方对账也停在 08-10/11。
- **目标**:跑 30 分钟以上同任务类型数据,量化 P0-P3 的实际收益(预期压缩后首轮命中率从 ~10% 显著提升);与官方小时级数据对账。
- **影响模块**:`scripts/analyze-cache-prefix.py` / stats 目录(只读分析,无代码改动)
- **工作量**:S-M(1-2 天):采集数据 → 跑分析 → 对账 → 更新基线文档。
- **验收**:产出 P0-P3 落地后的新基线数据(compacted/thinking/tail 三组),压缩后首轮命中率相对旧基线 ~10% 显著提升;与官方小时级数据对账误差可解释。
