...
| 缓存前缀稳定性 | ★★★★☆ | P0-P3 规则固化且代码/测试落地;**新基线已产出:稳定期 96.8%、压缩后首轮 89.6%(旧 ~10%),雪崩根治**(见 2026-08-16-P2落地后缓存命中新基线.md) |
...
### 方向 A:缓存前缀稳定性 P2 基线量化(验证雪崩根治收益)✅ 已达成

- **现状**:P0-P3 代码与测试已全部落地;新基线已产出(见 `.dsb/docs/2026-08-16-P2落地后缓存命中新基线.md`)。
- **结果(真实口径,provider_round,2026-08-15/16 共 1980 轮)**:
  - 稳定期命中率 **96.8%**(旧基线 68~75%)。
  - **压缩后首轮命中率 89.6%**(旧基线 ~10%),压缩雪崩根治。
  - 08-16 单会话长跑 831 轮 / 压缩 118 次,压缩后首轮仍 91.1%。
- **方法论修正**:真实 API 按块落盘,估算(消息粒度全有全无)偏悲观(08-16 估算 46.5% vs 真实 95%);`analyze-cache-prefix.py` 已修复 sendSeq 重置误配并新增真实口径分组,结论一律以真实口径为准。
...
| 优先级 | 方向 | 理由 | 工作量 | 依赖 |
|:------:|------|------|:------:|------|
| P0 | A 缓存前缀 P2 | 已达成(新基线:压缩后首轮 89.6%,雪崩根治) | 已完 | 无 |
