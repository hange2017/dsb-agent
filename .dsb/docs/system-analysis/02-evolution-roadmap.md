...
| 缓存前缀稳定性 | ★★★★☆ | P0-P3 规则固化且代码/测试落地;**新基线已产出:稳定期 97.0~97.7%、压缩后首轮 49~54%(compaction 后第一个 round,旧 ~10%;89.6% 为 30s 窗口虚高口径已废弃),雪崩大幅缓解**;2026-08-17 修正分析脚本并全量实测(见 2026-08-16-P2落地后缓存命中新基线.md + 2026-08-17-cache-prefix-analysis-fix.md) |
...
### 方向 A:缓存前缀稳定性 P2 基线量化(验证雪崩缓解收益)✅ 已达成

- **现状**:P0-P3 代码与测试已全部落地;新基线已产出(见 `.dsb/docs/2026-08-16-P2落地后缓存命中新基线.md`)。
- **结果(真实口径,provider_round,2026-08-15/16 共 1980 轮)**:
  - 稳定期命中率 **97.0~97.7%**(旧基线 68~75%,+22~29pp)。
  - **压缩后首轮命中率 49~54%**(compaction 后**第一个 round**,旧基线 ~10%,+40pp)——大幅缓解但未根治:命中部分 = system+tools+compacted 旧块(P2 生效),miss 部分 = 压缩后 tail 全部新消息(结构性成本)。
  - 08-16 单会话长跑 959 轮 / 压缩 132 次;旧文档"89.6%/91.1%"为 30s 窗口口径(把已恢复命中的轮次计入首轮)虚高,已废弃。
- **方法论修正(2026-08-17)**:真实 API 按块落盘,估算(消息粒度全有全无)偏悲观;`analyze-cache-prefix.py` 已修复 sendSeq 时间序配对/归组补 miss/broken 语义(新增 `--self-test` 六场景自检),08-16 估算 96.0% vs 真实 95.0% 高度吻合,结论一律以真实口径为准。
- **遗留**:官方小时级对账(08-10/11 之后)未重跑,可在下次数据采集时顺带完成。
...
| 优先级 | 方向 | 理由 | 工作量 | 依赖 |
|:------:|------|------|:------:|------|
| P0 | A 缓存前缀 P2 | 已达成(新基线:稳定期 97.0~97.7%、压缩后首轮 49~54%,雪崩大幅缓解) | 已完 | 无 |
