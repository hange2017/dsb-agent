...
> 状态:✅ 已完成(2026-08-17 首版;2026-09-14 增补)
> 评估时间点:2026-08-17(src 115 个 .ts、tests 107 文件 / 1012 项);
> 2026-09-14 复核:src 117 个 .ts(+taskMap.ts)、tests 110 文件 / 1165 项
...
| contextCompactor / contextManager | 4 | 3 | 5 | **3** | ★★★★ | **字节稳定性高风险区**;测试覆盖高。2026-09-14 增 taskMap 常驻地图 + 需求轨保护,风险面扩大(裁剪候选分轨需回归) |
| toolUsePolicy / toolResultPolicy | 5 | 4 | 5 | 1 | ★★★★★ | 职责单一,策略与执行分离;2026-09-14 增形状校验 + 字段分档(可重建/锚点) |
...
| **7** | ~~多轮压缩后目标漂移/含混~~ | **已于 2026-09-14 修复** | taskMap 常驻地图 + 需求轨保护 + 任务锚(工具轮可见)+ 运行时合成文本过滤 | ✅ 已闭环 |
| **8** | `context_recall` 回查率未量化 | 「模型是否真读回」缺证据 | 用 statsStore `context_recall` 事件统计「压缩后 N 轮内回查率」 | S |
| **9** | 阶段 C(96k/3:7)成本 +45.8%/$+62.9% | 每轮成本升高,tail ×2.15 为主因 | 回调 .40/.60 折中;或接受「少压缩、少漂移」 | S |
| **10** | map 轨历史遗留污染(累积式粘住旧 `[续写]` 行) | 旧会话地图残留合成文本 | 已加 `isRuntimeSyntheticText` 过滤;重载后再次压缩即清 | ✅ 已闭环 |
