 ...
 * 让 agent 不迷失目标。地图同时进入两处:
 *  1) 压缩块首段(永不 trim) —— 跨压缩保住的常驻地图;
 *  2) 每轮任务锚尾部(agentLoop 注入) —— 带最新清单的实时地图。
 * 同输入同输出(确定性),保证压缩块字节稳定、不破 KV 缓存前缀。
 */
...
export interface TaskMapInput {
   ...
   * 压缩块内地图无法访问 todo,故**不输出本段**,改由「更早的需求」承载信息。
   */
  next?: string[];
  /**
 ...
 * 被后续新需求挤出滑动窗口后永久消失(压缩块内地图永不裁剪,是天然的常驻位)。
 */
