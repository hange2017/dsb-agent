 ...
 *
 * P0-7(去命令式 + 去标签,2026-09-19):旧文案是**无条件命令句**「〔任务锚〕按下面清单继续推进」。
 * 用户中途下发新任务/中止时(实测:发布 v0.5.0 途中追加「停一下」并改做默认值改动),锚的命令与
 * 用户消息**互相冲突**,模型被迫每轮"裁决"一次并把结论写进正文 ——
 * 实测同一句反复出现:「任务锚仍要求发版 t49–t52;你上一轮已明确"停一下",发版不动。本轮做你要的默认值改动」。
 * 与 P0-6 同源(命令/条件句 → 每轮求值 → 元叙述回流),沿用同一套处置:
 * ① **去掉"继续推进"命令** → 改为对清单的**事实陈述**(「下列未完成项即本会话待办」);
 * ② 显式声明**用户消息优先于清单** → 冲突有确定答案,无需模型解释;
 * ③ **禁止在回复中说明清单与消息的关系**,并把出口从"写正文"改到"改清单"(TodoWrite);
 * ④ **去标签**:"任务锚"是可被引用的名词本身(实证复述句几乎都以它开头),改称不宣称权威的「会话计划」。
 * 语义等价:计划照旧可见、照旧是待办来源,只是不再向模型下命令。
 */
export const TASK_ANCHOR_HINT =
  "〔会话计划〕下列未完成项即本会话待办,供导航;用户消息优先于本清单,清单是否需要修订用 TodoWrite 处理,不必在回复中说明。需要更早的历史原文时,用 ContextRecall(seq=n) 回查压缩块中的 [r{n}] 行。";
...
/**
 * 「未完成项」段固定标题:由**真实未完成待办**生成(非历史需求)。
 * 单独成段是为了让模型每轮都能一眼看到"还没做什么",而不必自行从清单里筛 `- [ ]`。
 * P0-7:标题由「### 下一步」改为「### 未完成项」——前者隐含"接下来就做这些"的命令语气,
 * 用户中途改任务时会与用户消息打架;新标题只陈述**清单状态**,与锚文案的声明式基调一致。
 */
export const NEXT_STEP_TITLE = "### 未完成项";

/** 未完成项最多列出的条目数(超出只影响显示,不影响清单本身)。 */
const NEXT_STEP_MAX_ITEMS = 3;

/**
 * 由「未完成待办」的内容生成 `### 未完成项` 段;无未完成项返回空串。
 ...
 * 有值时注入 `### 未完成项` —— 这是锚里唯一宣称"待办"的地方,且来源是真实 todo 状态。
 */
...
export class AgentSession {
  ...
  constructor(
    private readonly deps: {
      ...
      /** 预算三块比例(压缩块/thinking/tail);缺省 20/0/80(两段:压缩块 20% / tail 80%)。 */
